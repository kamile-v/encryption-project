...
unsigned char shiftIndivMatrix[MATRIX_SIZE][MATRIX_SIZE] = {
    {0x02, 0x03, 0x01, 0x01},
    {0x01, 0x02, 0x03, 0x01},
    {0x01, 0x01, 0x02, 0x03},
    {0x03, 0x01, 0x01, 0x02}
...
unsigned char galoisField(unsigned char a, unsigned b) {
    ...
    for (int i = 0; i < 8; ++i) {
        if (b & 0x01) { 
            answer ^= a;           // answer += a in GF(2^8)
        }

        bool highBit = (a & 0x80); // a >= 128 = 0100 0000
        a <<= 1;                   // rotate a left (multiply by x in GF(2^8))
        if (highBit) {
            a ^= 0x1B;             // must reduce
        }
        b >>= 1;                   // rotate b right (divide by x in GF(2^8)
    }
